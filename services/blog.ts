...
export class BlogService{ 
    ...
    */
	public getPosts(paged:number=1){
		...
		if( this.posts && this.config.trackComponents('allposts') >= paged*this.config.per_view){

			if(this.posts.length >= paged*this.config.per_view){            
			...
			let url = `${this.config.baseUrl}blog/?per_page=`+this.config.per_view+'&page='+paged;
