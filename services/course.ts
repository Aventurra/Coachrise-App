...
export class CourseService implements OnInit{  
    ...
    getAllCourses(paged:number = 1){
         ...
         if(this.config.trackComponents('allcourses') >= paged*this.config.per_view){
            if(this.courses.length >= paged*this.config.per_view){            
            ...
            let count:number=0;
            count = paged*this.config.per_view - this.courses.length;
    ...
    getAllCourseCategory(){
        ...
        if(this.config.trackComponents('allcoursecategories')){
            ...
            .map(response =>{
                ...
                if(body){
                    for(let k =0; k< body.length;k++){
                        this.courseCategories.push(body[k]);
                    }
    ...
    getAllCourseLocations(){
        ...
        if(this.config.trackComponents('allcourselocations')){
            ...
            .map(response =>{
                ...
                if(body){
                    for(let k =0; k< body.length;k++){
                        this.courseLocations.push(body[k]);
                    }
    ...
    getAllCourseLevels(forceRefresh?: boolean){
        ...
        if(this.config.trackComponents('allcourselevels')){
            if(this.courseLevels.length){
                return Observable.of(this.courseLevels);
            }else{
                this.locobservable = Observable.fromPromise(this.storage.get('allcourselevels').then((cats) => {
                    if(cats){
    ...
    getPopularCourses(){
        ...
        if(this.config.trackComponents('popular')){
            ...
            this.popularobservable = this.http.get(`${this.baseUrl}course/popular`+'?per_view='+this.config.per_view)
            .map(response =>{
                let body = response.json();
    ...
    */
    ...
    getFeaturedCourses(){
        ...
        if(this.config.trackComponents('featured')){
            ...
            this.featuredobservable = this.http.get(`${this.baseUrl}course/featured?per_view=`+this.config.per_view)
            .map(response =>{
                let body = response.json();
                if(body){
                    let courses = [];
    ...
    filterItems(selectFilters:any){
        ...
        let url = `${this.baseUrl}course/filters?filter=`+encodeURI(JSON.stringify(selectFilters));
        let cacheKey = url;
        console.log(url);
        let items:any[]=[];
        ...
        if(this.config.trackComponents('allcourses') == this.courses.length && this.courses.length > 0){
            ...
            if(items.length < this.config.per_view){
