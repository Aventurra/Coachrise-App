...
export class UserService{  
    ...
    public getProfileTab(user_id:any,key:string,force:boolean=false){
        ...
        if(loadedtabs.indexOf(key) == -1 || force){
            ...
            if(this.tabobservable[key]) {
                ...
                let opt = this.auth.getUserAuthorizationHeaders();
                let url = `${this.baseUrl}user/profile/?tab=`+key+`&per_page=`+this.config.per_view+`&paged=`+this.profileTabPaged[key];
    ...
    public getMoreProfileTab(user,key){
        ...
        return this.http.get(`${this.baseUrl}user/profile/?tab=`+key+`&per_page=`+this.config.per_view+`&paged=`+this.profileTabPaged[key],opt)
        .map(response =>{
            this.observable = null;                    
            if(response.status == 400) {
              return "FAILURE";
